...
class Output
{
    ...
    public function load(string $route, array $data = array()): void
    {
        ...
        // Nếu chưa đăng nhập, buộc chuyển hướng đến trang login trước khi tải header và footer
        if (!isset($_SESSION['user_logged_in'])) {
            echo $this->loadFile('src/login/view/loginView.php', $data);
            exit; // Dừng việc tải tiếp header/footer
        }
        ...
        $module = $route_paths[0];  // Lấy tên module (VD: 'book')
        $view = $route_paths[1];     // Lấy tên view (VD: 'update')
        // Tự động thêm CSS & JS cho module
        $this->addCss($module, $view);
        $this->addJs($module, $view);
